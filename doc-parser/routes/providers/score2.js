...
let good = ["sehr","äusserst","immer","stets","jederzeit","absolut","gleichermassen","gleichermassen sehr","ausserordentlich","hoch","weiterhin"]
...
function score(body, annotations) {
    ...
    annotations.entities.forEach(function (value, index, a) {
        if (verhalten.includes(value.name)) {
            console.log("" + value.name + " mentioned at " + value.mentions[0].text.beginOffset)
            console.log(annotations.sentences[0].sentiment.score, annotations.sentences[0].sentiment.magnitude )
        }
        // console.log(value);
    })
...
let body = "Das freundliche korrekte und zuvorkommende Verhalten von Frau De Bon war stets gut.";
let annotations =
    {
        "sentences": [
            {
                "text": {
                    "content": "Das freundliche korrekte und zuvorkommende Verhalten von Frau De Bon war stets gut.",
                    "beginOffset": 0
                },
                "sentiment": {
