...
function analyzeLetterStructure(parsedFile, res) {
    ...
    async.each(paragraphsMetaConfig,(paragraphConfig, cb) => {
        async.eachOf(parsedFile.paragraphs, (paragraph, index, cb2) => {
            ...
            .then(results => {
                ...
                _.forEach(paragraphConfig.metaWords,(metaWord) => {
                        syntax.tokens.forEach(part => {
                            if(natural.LevenshteinDistance(metaWord.toLowerCase(), part.text.content.toLowerCase()) < 2){
                                  
                                found.push(part.text.content); 
                                score++;
                            }
                        });
                ...
                _.forEach(paragraphConfig.metaMix,(metaMix) => {
                    if(natural.LevenshteinDistance(metaMix.toLowerCase(), paragraph.toLowerCase(), {search: true}).distance < 2){
                        found.push(metaMix);
                        score++;
                    }
                })
                ...
                // console.log(paragraphsMetaData)
